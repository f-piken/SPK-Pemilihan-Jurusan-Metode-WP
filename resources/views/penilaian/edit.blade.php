    ...
    <section class="section">
      <div class="row" >
        <div class="col-lg-6" style="width: 1500px;">
          <a href="{{ route('penilaian.index') }}" class="btn btn-primary mb-3">Data Kriteria</a>
          <div class="card">
            <div class="card-body">
              <form action="{{ route('penilaian.update', $penilaian->id_penilaian) }}" method="POST">
                    ...
                    <div class="mb-3">
                      <label for="id_mahasiswa" class="form-label">Mahasiswa</label>
                        <select name="id_mahasiswa" id="id_mahasiswa" class="form-select">
                            ...
                            <option value="{{ $mhs->id_mahasiswa }}" {{ $penilaian->id_mahasiswa == $mhs->id_mahasiswa ? 'selected' : '' }}>
                              {{ $mhs->nama }}
                          </option>
                            @endforeach
                      </select>
                    ...
                    <div class="mb-3">
                      <label for="id_jurusan" class="form-label">Jurusan</label>
                        <select name="id_jurusan" id="id_jurusan" class="form-select">
                            ...
                            <option value="{{ $jrs->id_jurusan }}" {{ $penilaian->id_jurusan == $jrs->id_jurusan ? 'selected' : '' }}>
                              {{ $jrs->nama_jurusan }}
                          </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                      <label for="id_kriteria" class="form-label">Kriteria</label>
                        <select name="id_kriteria" id="id_kriteria" class="form-select">
                            ...
                            <option value="{{ $ktr->id_kriteria }}" {{ $penilaian->id_kriteria == $ktr->id_kriteria ? 'selected' : '' }}>
                              {{ $ktr->nama_kriteria }}
                          </option>
                            @endforeach
                        </select>
                    <div class="mb-3">
                        ...
                        <input class="form-control" type="number" name="nilai" id="nilai" min="0" max="100" value="{{ $penilaian->nilai }}">
                    </div>
